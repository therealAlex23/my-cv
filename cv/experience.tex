...
%---------------------------------------------------------
  \cventry
	...
	\item {Implemented DMA transfers to mirror physical data to resources in a periodic monitoring task; defined MMU pages with permissions and sizes based on the overall mapping for dedicated memory sections to applications and BMIs, and updated linker scripts to ensure precise static allocation;}
	\item {Performed formal mathematical verifications (Unit Proof) based on Frama-C/ACSL contracts, alongside integration testing using Robot Framework with eRPC stubs to flash the Firmware/Software and stimulate hardware interfaces.}
	...
	\item {Accelerated development workflows by engineering custom Python and Shell scripts for local process automation.}
      \end{cvitems}
    }
